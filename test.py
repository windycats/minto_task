...
wd.implicitly_wait(10)
wd.find_element_by_xpath('/html/body/div[3]/table/tbody/tr[2]/td[2]/div/table/tbody/tr[3]/td/div[2]/input[2]').click()

act = ActionChains(wd)
act.move_to_element(wd.find_element_by_xpath('//div[contains(text(),"任务管理")]')).perform()  # 定位任务管理的
WebDriverWait(wd, 10).until(lambda x: x.find_element_by_xpath('//*[@title="工作周报"]'))  # 定位年度重点工作
wd.find_element_by_xpath('//*[@title="工作周报"]').click()
sleep(2)
...
wd.find_element_by_xpath('//*[@title="选择责任单位"]').click()  # 点击承办单位的框框
wd.find_element_by_xpath('//*[@field_name="content"]').send_keys('这是任务目标')  # 任务目标
wd.switch_to.default_content()
